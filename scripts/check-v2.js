...
const reviews = [
  ...
  [/function shouldUseNativeNoiseSuppression\(\) \{\s*return dom\.noiseSuppressionToggle\.checked;\s*\}/.test(app), "native noise suppression follows the toggle regardless of gate strength"],
  [/const noiseGate = dom\.noiseSuppressionToggle\.checked && getNoiseGateStrength\(\) > 0;/.test(app), "backup noise gate layers on top of native suppression instead of replacing it"],
  ...
  [main.includes('"list-screen-windows"') && preload.includes("listScreenWindows") && app.includes('surface === "monitor" || state.screenWindow') && /if \(state\.screenWindow\) \{[\s\S]{0,300}return getElectronDesktopScreenShareStream\(state\.screenWindow\.id\);/.test(app), "windows screen share can capture one program window without falling back to the whole monitor"],
