...
const reviews = [
  ...
  [/getSystemAudioStreamOrNull\("Windows process loopback"[\s\S]+getSystemAudioStreamOrNull\("Windows display loopback"/.test(systemAudioDisplayFunction) && helperSource.includes('"--exclude"') && main.includes('"--exclude"') && preload.includes("excludeSelf: true"), "windows system share tries self-excluding process loopback before endpoint loopback"],
