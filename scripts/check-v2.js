...
const reviews = [
  ...
  [main.includes('"list-screen-windows"') && preload.includes("listScreenWindows") && app.includes('surface === "monitor" || state.screenWindow') && /if \(state\.screenWindow\) \{[\s\S]{0,300}return getElectronDesktopScreenShareStream\(state\.screenWindow\);/.test(app), "windows screen share can capture one program window without falling back to the whole monitor"],
