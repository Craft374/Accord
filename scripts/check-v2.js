...
const checks = [
  ...
  [app.includes("getLegacyWebRtcProcessingConstraints") && app.includes("googEchoCancellation"), "chromium webrtc processing hints exist"],
  ...
  [app.includes("selectSeparatedWindowsOutputForSystemShare") && app.includes("isLikelyHeadsetLoopbackOutputLabel"), "windows headset output is separated during system share"],
  ...
  [app.includes("getWindowsAudioRoutingIssue") && app.includes("출력 분리"), "windows headset recapture risk is surfaced"],
...
const reviews = [
  ...
  [/if \(dom\.echoCancellationToggle\.checked\)[\s\S]+enabled: false/.test(app) === false, "fallback processing remains available while echo cancellation is requested"],
  ...
  [/getMicConstraints\(\)[\s\S]+getAudioProcessingAdvancedConstraints\(echo, noise, autoGain\)/.test(app), "mic capture requests advanced speech processing"],
  [/enforceMicProcessingConstraints\(\)[\s\S]+getLegacyWebRtcProcessingConstraints\(echo, noise, autoGain\)/.test(app), "runtime mic constraints include chromium processing hints"],
  [/getDisplayMedia\([\s\S]+getLegacyWebRtcProcessingConstraints\(false, false, false\)/.test(app), "display system audio disables chromium speech processing"],
  [/getSystemInputConstraints\(\)[\s\S]+getLegacyWebRtcProcessingConstraints\(false, false, false\)/.test(app), "virtual system audio disables chromium speech processing"],
  ...
  [/parts\.push\("AEC후"\)/.test(app), "processing status shows post-AEC backup processing"],
